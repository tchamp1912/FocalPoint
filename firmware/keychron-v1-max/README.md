...
|-------|---|-------------|----------|
| `MAC_BASE` | 0 | default (Mac switch) | hold Option with a control key; normal chords pass through; left Ctrl/Cmd swapped |
| `MAC_FN`   | 1 | hold stock Fn | stock |
| `WIN_BASE` | 2 | OS switch = Win | hold Alt with a control key; normal chords pass through |
| `WIN_FN`   | 3 | hold stock Fn | stock |
| `FOCALPOINT`  | 4 | **hold Option/Alt** | control surface below |

FocalPoint is transparent everywhere except its documented controls. Holding
Option/Alt with another normal key therefore preserves the OS shortcut:
